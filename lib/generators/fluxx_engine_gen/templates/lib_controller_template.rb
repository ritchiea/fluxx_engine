module FluxxGrantRequestsController
  ...
  def self.included(base)
    base.insta_index <%= controller_class_name %> do |insta|
    ...
    base.insta_show <%= controller_class_name %> do |insta|
      insta.template = '<%= controller_class_singular_table_name %>_show'
      insta.icon_style = ICON_STYLE
      insta.add_workflow
    end
    base.insta_new <%= controller_class_name %> do |insta|
      insta.template = '<%= controller_class_singular_table_name %>_form'
      insta.icon_style = ICON_STYLE
    end
    base.insta_edit <%= controller_class_name %> do |insta|
      insta.template = '<%= controller_class_singular_table_name %>_form'
      insta.icon_style = ICON_STYLE
    end
    base.insta_post <%= controller_class_name %> do |insta|
      insta.template = '<%= controller_class_singular_table_name %>_form'
      insta.icon_style = ICON_STYLE
    end
    base.insta_put <%= controller_class_name %> do |insta|
      insta.template = '<%= controller_class_singular_table_name %>_form'
      insta.icon_style = ICON_STYLE
      insta.add_workflow
    end
    base.insta_delete <%= controller_class_name %> do |insta|
      insta.template = '<%= controller_class_singular_table_name %>_form'
      insta.icon_style = ICON_STYLE
    end
    base.insta_related <%= controller_class_name %> do |insta|
