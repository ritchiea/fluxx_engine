module Rack
  class Scrap
    ...
    def self.config
      @@config ||= YAML::load open([Rails.root, "config", "scrap.yml"].join('/')).read
    rescue Errno::ENOENT
      @@config = {}
    rescue
    ...
    def call env
      ...
      @@request_list.pop if @@request_list.length > (Rack::Scrap.config["max_requests"] || 150)
    ...
    def gc_stats    
      ...
      s << "<h3>Top #{Rack::Scrap.config["max_objects"]} deltas since last request</h3>"
      s << '<table border="0">'
      memcheck(Rack::Scrap.config["max_objects"], Object, :deltas).each do |v|
        next if v.last == 0
        s << "<tr><td class='t'>#{v.first}</td><td>#{sprintf("%s%s", v.last >= 0 ? "+" : "-", commify(v.last))}</td></tr>"
      end
      s << '</table>'

      s << "<h3>Top #{Rack::Scrap.config["max_objects"]} objects</h3>"
      s << '<table border="0">'
      memcheck(Rack::Scrap.config["max_objects"]).each do |v|
        s << "<tr><td class='t'>#{v.first}</td><td>#{commify v.last}</td></tr>"
      end
      s << '</table>'
    
      (Rack::Scrap.config["classes"] || {}).each do |klass, val|
        puts val.inspect
