    ...
    if (MultiElementGroup.exists? rescue false)
      ...
      groups.each do |group|
        if blank_model_obj.respond_to? "#{group.name.singularize}_id"
          p "ESH: ModelDslMultiElement, model_class.belongs_to #{group.name.singularize.to_sym}, :class_name => 'MultiElementValue', :conditions => {:multi_element_group_id => group.id}"
