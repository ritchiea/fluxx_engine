...
class ActiveRecord::ModelDslTemplate < ActiveRecord::ModelDsl
  ...
  def evaluate_model_method model, method_name
    model.send(method_name) if method_allowed?(model, method_name)
  end
  
  def evaluate_model_list_method model, method_name
    p "ESH: calling method_name #{method_name} on model #{model}, extra_list_methods=#{extra_list_methods.inspect}"
  ...
  # This will replace fluxx_iterator, fluxx_conditional, fluxx_value elements within a DOM generated by Nokogiri
  # It will return a Nokogiri DOM with all elements resolved or throw an exception
  MAX_DEPTH = 100
