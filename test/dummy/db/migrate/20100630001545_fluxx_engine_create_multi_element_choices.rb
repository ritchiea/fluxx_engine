class FluxxEngineCreateMultiElementChoices < ActiveRecord::Migration
  def self.up
    ...
    execute "alter table multi_element_choices add foreign key multi_element_value_id references multi_element_values(id)" unless connection.adapter_name =~ /SQLite/i
  end

  def self.down
    remove_foreign_key 'multi_element_choices', 'multi_element_values', 'mult_elt_choices_mult_elt_val_id'
